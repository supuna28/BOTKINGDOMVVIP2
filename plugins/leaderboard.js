let handler = async (m, { conn, args, participants }) => {
  ...
  let text = `
┌「 *Leaderboard Top ${len}* 」
├ තොරතුරු: *${usersExp.indexOf(m.sender) + 1}* ගනන*${usersExp.length}*
│
${sortedExp.slice(0, len).map(({ jid, exp }, i) => `├ ${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *${exp} Exp*`).join`\n`}
└────
┌「 *Leaderboard Top ${len}* 」
├ තොරතුරු: *${usersLim.indexOf(m.sender) + 1}* ගනන *${usersLim.length}*
│
...
└────
┌「 *Leaderboard Top ${len}* 」
├ තොරතුරු: *${usersLevel.indexOf(m.sender) + 1}* ගනන *${usersLevel.length}*
│
